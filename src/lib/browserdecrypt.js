...
export default class Decryptor {
	...
	decryptMetadata(data) {
		...
		let header = sodium.from_base64(
			data.header,
			sodium.base64_variants.ORIGINAL_NO_PADDING
		);
		let encryptedMetadata = sodium.from_base64(
	...
	decrypt(data, isLast = false) {
		...
		let {
			message,
			tag,
		} = sodium.crypto_secretstream_xchacha20poly1305_pull(this.state, data);

		let expectedTag = isLast
